...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   map_section_analysis_00.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: oel-houm <oel-houm@student.1337.ma>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/07 01:11:47 by oel-houm          #+#    #+#             */
/*   Updated: 2024/01/08 18:28:10 by oel-houm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_gaps(t_map_info *map_data, int start, int end)
{
	...
	while (start < end)
	{
		...
		while (map_data->map_content[start][i])
		{
			...
			{
				printf(
					"[ERROR]: Unexpected chracter '%c' found in the map \
					at line %d index %d.\n",
...
void	start_texture_to_end_texture(char **map, int i, int end)
{
	while (map[i] && i < end)
	{
		...
		{
			printf(
				"[ERROR]: Unexpected data found in the texture configuration at \
				line %d. The entry '%s' is not recognized.\n", i + 1, map[i]);
			exit(1);
		}
	}
}
...
void	validate_player_chracter_count(int *character)
{
	...
	{
		printf(
			"[ERROR]: More than one character (player) detected on the map.\n");
		exit(1);
	}
	else if (*character == 0)
	{
	...
	while (start <= end)
	{
		...
		while (line[start][i])
		{
			if (validate_characters(line[start][i], &character) == 0)
			{
				printf(
					"[ERROR]: Unexpected chracter '%c' found in the map \
					at line %d index %d.", line[start][i], start + 1, i);
				exit(1);
			}
			i++;
		}
...
void	sections_gaps_analysis(t_map_info *map_data)
{
	...
	start_texture_to_end_texture(map_data->map_content,
		map_data->texture_start_index, map_data->texture_end_index);
	check_gaps(map_data, map_data->texture_end_index + 1,
		map_data->map_start_index);
	check_gaps(map_data, map_data->map_end_index + 1,
		map_data->eof_index + 1);
	start_map_to_end_map(map_data->map_content, map_data->map_start_index,
